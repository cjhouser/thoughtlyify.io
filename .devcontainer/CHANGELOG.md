...
## 0.15.0
### Added
- `devenv` alias which switches the terminal's kubectl context to the local kind cluster running on the same Podman VM as the devcontainer
- trixie repositories now have azure-cli

## 0.14.0
### Changed
- update terraform shortcuts to manage state, plans, and secrets within a volume that is separate from the repositories

## 0.13.2
### Added
- `gitr` bash function. Takes an optional branch name as an argument and defaults to main branch if unspecified.
- `destroy` bash function to destroy root modules.
### Fixed
- missing locale `en_US.UTF-8` caused warnings to appear in terminal when bash was started and during certain command execution
### Removed
- `gitr` alias
